...
/opt/bin/generate_extras_config > /opt/selenium/selenium_grid_extras_config.json
...
#update the current config.json with current $HOSTNAME
jq '.host=env.HOSTNAME' /opt/selenium/config.json > tmp.json && mv tmp.json /opt/selenium/config.json
